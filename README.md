...
Sleep-Parse-Plugin 是一个专门用于解析能斯达智能睡眠监测产品设备睡眠数据的工具类库，提供了简洁的静态方法来解析不同格式的睡眠数据，便于客户直接调用，降低接入门槛。
...
1. 克隆项目到本地
   ```bash
   git clone https://github.com/jyehui6/sleep-parse-plugin.git
   cd sleep-parse-plugin
   ```
